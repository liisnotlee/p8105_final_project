...
```
```{r}
ggplot(my_int_numeric, 
       aes(x = phase, y = moodswing_num, group = id, color = factor(id))) +
  ...
  guides(color = "none")
  labs(
    title = "Individual Mood Swing Patterns Across Menstrual Phases",
    x = "Menstrual Cycle Phase",
    y = "Mood Swing Intensity (0–5)"
  ) +
  theme_minimal() +
  theme(
...
reletionship between hormone and 

```{r}
my_i = 
  hormone |>
  select(id, phase, lh, estrogen, fatigue, moodswing, flow_color, flow_volume, appetite) |>
  mutate(
    phase = factor(phase, levels = c("Follicular", "Fertility", "Luteal", "Menstrual")),
    flow_volume = factor(flow_volume, 
                         levels = c("Not at all", "Very Heavy", "Somewhat Heavy", "Moderate", "Light", 
                                    "Spotting / Very Light", "Heavy")),
    flow_color = factor(flow_color,
                        levels = c("Not at all", "Bright Red", "Dark Brown / Dark Red", "Grey", 
                                   "Pink", "Other", "Black")),
    appetite = factor(appetite,
                      levels = c("Low", "Very Low", "Moderate", "High", "Very High"))
  )   
